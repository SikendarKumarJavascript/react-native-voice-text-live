...
export default function App() {
  ...
  const [permissionStatus, setPermissionStatus] =
    useState<PermissionResult>("denied");
  ...
  useEffect(() => {
    ...
    speechErrorSubscription = addSpeechErrorListener((event) => {
      ...
      // Filter out common operational errors that are not critical
      ...
      // Auto-restart listening if it's just no match (user can keep trying)
      if (
        event.error.includes("No match found") ||
        event.error.includes("No speech input")
      ) {
        // Keep listening state true so user can try again
        console.log("No speech detected, ready for input...");
      } else {
  ...
  const handleSpeak = async () => {
    if (!transcribedText) {
      Alert.alert(
        "No Text",
        "Please transcribe some text first before using text-to-speech.",
      );
      return;
    }
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.transcriptionContainer}>
          ...
          <Text style={styles.transcriptionText}>
            {transcribedText ||
              "Start listening to see transcribed text here..."}
